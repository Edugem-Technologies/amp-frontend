...
// ICON MAP — har object ke liye alag
const iconMap: Record<string, JSX.Element> = {
    // Marketplace
    XYZ: <PeopleIcon />,
    Splink: <ShoppingCartIcon />,
    FlexEcom: <ApartmentIcon />,
    Outmin: <BarChartIcon />,
    Linkie: <TrendingUpIcon />,

    // Meetings
    "Meeting Schedule": <EventNoteIcon />,
    "UI Conference": <EventNoteIcon />,
    "03 May 2020": <EventNoteIcon />,
    ...
    // BusinessStats
    sapProgress: <TrendingUpIcon />,
    newCustomers: <PeopleIcon />,
    milestoneReached: <TrendingUpIcon />,
    milestoneReachedBar: <TrendingUpIcon />,

    // UserDetails
    "Arthur Goldstain": <PersonIcon />,
    "Lisa Bold": <PersonIcon />,
    "Nick Stone": <PersonIcon />,

    // KPIs
    projectProgress: <TrendingUpIcon />,
    companyFinance: <BarChartIcon />,
    marketingAnalysis: <PeopleIcon />,
    ...
    return (
        <div className="dashboard-container d-flex flex-column gap-4">
            ...
            <div className="sales-grid">
                {Object.entries(dashboardData.salesStats).map(([key, stat]) => (
                    <KpiCard
                        key={key}
            ...
            >
                {Object.entries(dashboardData.businessStats).map(([key, stat]) => (
                    <KpiCard
                        key={key}
                        title={stat.desc}
            ...
            <div className="user-grid" style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
                {dashboardData.userDetails.map((user) => (
                    <KpiCard
                        key={user.name}
                        title={user.name}
                        amount=""
                        description={user.designation}
                        icon={iconMap[user.name] || iconMap.default}
                        iconPlacement="left"
            ...
            <div className="kpi-grid" style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
                {dashboardData.kpis.map((kpi) => (
                    <KpiCard
                        ...
                        showProgress
                        icon={iconMap[kpi.id] || iconMap.default}
                        iconPlacement="top"
