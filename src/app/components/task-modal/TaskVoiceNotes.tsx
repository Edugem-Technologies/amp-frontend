...
const TaskVoiceNotes = ({ voiceNotes }: TaskVoiceNotesProps) => {
    return (
        <Accordion className="v1 my-6">
            <Accordion.Item eventKey="0">
                ...
                <Accordion.Body>
                    {voiceNotes.map((note) => (
                        <div key={note.id} className="card card-bordered p-4 mb-4">
                            <div className="d-flex align-items-center justify-content-between mb-3">
                                <div>
                                    <div className="fw-bolder fs-6">{note.title}</div>

                                    <div className="text-gray-400 fs-7">{note.duration}</div>
                                </div>
                            </div>
