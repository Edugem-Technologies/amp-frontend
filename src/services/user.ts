...
export const doGetUserByAccessToken = async (token: string) => {
    const url = new URL(
        process.env.NEXT_PUBLIC_API_URL + `/${config.API_VERSION}/user/get-user-by-token`,
    )
...
export const doVerifyUserEmail = async (data: UserEmailVerifyPropType) => {
    const url = new URL(
        process.env.NEXT_PUBLIC_API_URL + `/${config.API_VERSION}/user/confirm-verification-code`,
    )
